...
//	private FJTaskRunnerGroup runnerGroup = null;
	...
	public void prepareInitialState() throws InvalidSpecificationException {
		...
		shouldPrintProcessorStats = (capi.getProperty(EngineProperties.PRINT_PROCESSOR_STATS_PROPERTY, "no").toUpperCase().equals("YES"));
	...
	*/
	...
	public synchronized void retrieveAgents() {
		...
		if (agentSetFlat instanceof Enumerable) {
            if (stepCount < 1) {
                ...
                for (Element agent: ((Enumerable)agentSetFlat).enumerate()) {
                	Location loc = new Location(AbstractStorage.PROGRAM_FUNCTION_NAME, ElementList.create(agent));
                	try {
		...
		/* 
		 * We want to group the schedules for the whole run, 
		 * so we set the group handle to be this instance of scheduling
		 * policy that we are using in this run.
		 */
	...
	public void chooseAgent() {
        // if there are no more agents, we are done this step
		if (selectedAgentSet.size() == 0) {
			chosenAgent = null;
		}
        else if (selectedAgentSet.size() == 1) {
            ArrayList<Element> agentsList = new ArrayList<Element>(selectedAgentSet);
            chosenAgent = agentsList.get(0);
        }
		else {
			Random random = new Random();
            
            // chose an agent from the selected agent set randomly
			int chosenAgentIndex = random.nextInt(selectedAgentSet.size());
			
			// this may be inefficient
			ArrayList<Element> agentsList = new ArrayList<Element>(selectedAgentSet);
			chosenAgent = agentsList.get(chosenAgentIndex);			
		}
		
		// debugged by Roozbeh Farahbod, 18-Jan-2006
		if (chosenAgent != null) {
	...
	*/
	...
	public void executeAgentPrograms() throws EngineException {
		 ...
		 * Old Code
		 * 
		 * Before, we would keep a copy of the runnerGroup and reuse it.
		 * This appeared to be problamatic with regard to memory issues.
		 * Changing it to one instance per time...
		 *
		if (runnerGroup == null) {
			//TODO the number of available processes may change!
			int cpus = getNumberOfProcessorsToBeUsed();
			batchSize = getThreadBatchSize();
	        runnerGroup = new FJTaskRunnerGroup(cpus);
	        if (Logger.verbosityLevel == Logger.INFORMATION) {
	        	Logger.log(Logger.INFORMATION, Logger.scheduler, "Using " + cpus + " thread(s) on " + Runtime.getRuntime().availableProcessors() + " processors.");
		...
		*/
		...
		ConcurrentProgramEvaluator cpe = new ConcurrentProgramEvaluator(capi, agentContextMap, agentsList, 0, agentsList.size(), batchSize);
		try {
			runnerGroup.invoke(cpe);
		} catch (InterruptedException e) {
			throw new EngineException("Could not finish program evaluation due to " +
...
//			capi.error(cpe.getError());
	...
	public void accumulateUpdates() {
		// Edited by Roozbeh Farahbod, 18-Jan-2006
		Logger.log(Logger.INFORMATION, Logger.scheduler,
				"Accumulating updates.");
		Collection c = chosenProgram.getBody().getUpdates();
		
		// 'if' Added by Roozbeh Farahbod, 29-Sep-2006
		if (c == null) {
			String msg = "Rule '" + chosenProgram.getName() + "' provides no updates.";
			Logger.log(Logger.ERROR, Logger.scheduler, msg);
			throw new EngineError(msg);
		}
			
		updateInstructions.addAll(c);
		Logger.log(Logger.INFORMATION, Logger.scheduler,
				"Updates are: " + c.toString());
	}
	*/
	...
	public void initiateExecution() {
		...
		// 'initiateProgramExecution' replaces the clearTree method call
		// to allow the interpreter to perform internal initialization 
		// prior to program execution
		capi.getInterpreter().initProgramExecution();
	...
	*/
	...
	public boolean isSingleAgentInconsistent() {
		...
		Set<Update> inconsistentUpdates = capi.getStorage().getLastInconsistentUpdate();
	...
	*/
	...
	/*
	 * Looks into scheduler plugins and find a scheduling 
	 * policy or creates a default one.
	 */
	private void loadSchedulingPolicy() {
		...
		if (possiblePolicies.size() == 0)
			...
			if (possiblePolicies.size() == 1)
				...
				if (possiblePolicies.size() > 1) {
					throw new EngineError("Conflicting scheduling policies provided by " + providers + ".");
				}
	}
	
	/*
	 * Based on the number of processors on the machine and the limit
	 * set by the user, returns the number of processors to be used
	 * for simulation.
	 */
	...
	/*
	 * Returns the user defined value for the minimum number of agents
	 * to be assigned to every execution thread during the simulation.
	 */
	private int getThreadBatchSize() {
		int size = 1;
		String sizeStr = capi.getProperty(EngineProperties.AGENT_EXECUTION_THREAD_BATCH_SIZE, "1");
		try { 
