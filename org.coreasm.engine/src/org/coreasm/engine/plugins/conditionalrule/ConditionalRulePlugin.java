...
//        opRules.add(new OperatorRule(CONDITIONAL_OP1,CONDITIONAL_OP2,
//                OpType.TERNARY,
//                10.0,
//                OpAssoc.RIGHT,
//                getName()));
//        
    ...
    public Element interpretOperatorNode(Interpreter interpreter, ASTNode opNode) throws InterpreterException {
        if (opNode.getToken().equals(OperatorContributor.getOpSymbolsAsString(CONDITIONAL_OP1,CONDITIONAL_OP2))) {
            if (opNode.getFirst().getValue().equals(BooleanElement.TRUE)) {
                return opNode.getFirst().getNext().getValue();
            }
            
            if (opNode.getFirst().getValue().equals(BooleanElement.FALSE)) {
                return opNode.getFirst().getNext().getNext().getValue();
            }
