...
of \code{Agents}\index{Agents@\code{Agents}}. A function called \code{program}\index{program@\code{program}} is also defined in the kernel which 
...
\bExample
	\codebf{choose} p \codebf{in} prices \codebf{with} 
		(\codebf{forall} pi \codebf{in} prices \codebf{holds} p <= pi) \codebf{do}\index{choose rule@\keyword{choose} rule} \\
	\tab \codebf{print} p
\eExample
...
\ruleform{\codebf{if} \val \codebf{then} \crule}{Conditional Rule Plugin}\index{conditional rule}\index{if@\keyword{if-then-else}|see{conditional rule}}
