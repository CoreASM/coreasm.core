...
%\newcommand{\asm}[1]{\xspace{\bf \code{#1}}\xspace}
%\newcommand{\asm}[1]{\asm{#1}}
...
%\renewcommand{\asm{id}}{{\em id}\xspace}
%\renewcommand{\asm{loc}}{{\em loc}\xspace}
%\newcommand{\asm{value}}{{\em value}\xspace}
%\newcommand{\asm{rule}}{{\em rule}\xspace}
%\newcommand{\asm{guard}}{{\em guard}\xspace}
\newcommand{\rulei}[1]{{\em rule}$_{#1}$\xspace}
%\newcommand{\asm{id}i}[1]{{\em id}$_{#1}$\xspace}
%\newcommand{\asm{value}uei}[1]{{\em value}$_{#1}$\xspace}
%\newcommand{\asm{value}uepi}[1]{{\em value}$'_{#1}$\xspace}
%\newcommand{\asm{loc}i}[1]{{\em loc}$_{#1}$\xspace}
%\newcommand{\tab}{$~~~~$}
...
This operator returns the rule element of a rule with the given name (\asm{id}). \new{Rule element is an element in the CoreASM state
that represents a rule defined in the specification.} It is useful in 
assigning rules to programs of agents. In the following example, \asm{Main} is the name 
of a rule:
...
\opform{
	...
	\index{not equal@\asm{!=} operator|see{not-equal operator}}
...
\opform{\asm{not} \asm{value}}{Predicate Logic Plugin}\index{not@\asm{not} operator}
...
The resulting update set is a sequential composition of the updates generated by \asm{rule$_1$} and \asm{rule$_2$}.
The optional keyword \asm{next} is meant to improve readability specially where the sequence rule is combined 
with other rule forms. 

\ruleform{
	\asm{seqblock rule$_1$}
...
Similar to the \asm{seq} rule (above), this rule form executes the listed rules in sequence. It evaluates \asm{rule$_1$}, 
applies the generated updates in a virtual state, evaluates \asm{rule$_2$} in that state and applies the generated 
...
evaluates the rule. \new{In the ASM book this is written as "\asm{loc <- rule}".}
...
If \asm{loc} is a location in the state (e.g., a function) and its value is a set, this rule 
produces an update instruction (partial update) that adds \asm{value} to \asm{loc}.
...
\index{list element}
\index{[]@\asm{[\ldots]}|see{list element}}
...
The function \asm{zipwith} generalises zip by zipping with the function given as the last argument, instead of a tupling function. For example, \asm{zipwith (l1, l2, @Sum)}  is applied to two lists to produce a list of corresponding maximums, where \asm{derived Sum(x, y) = x + y}.
...
\index{map element}
\index{[]@\asm{[\ldots]}|see{map element}}
...
the functionality provided is limited to introducing an \asm{include} keyword that would load another file 
into the current specification. 

%\begin{quote} \asm{ \asm{include} {\em filename}} \end{quote}\index{include@\asm{include}}
\fxnote{removed}
Included files can themselves have other \asm{include} clauses to further break down 
the specification.
...
%\begin{quote} \asm{ \asm{option} {\em property} {\em value} } \end{quote}\index{option@\asm{option}}
\fxnote{removed}
...
%\begin{quote} \asm{ \asm{option} \asm{SchedulingPolicies.policy} {\em policyname} } \end{quote}
\fxnote{removed}
Currently, there are two scheduling policies provided by this plugin:
\begin{itemize}
  \item \asm{allfirst} Tries executing all the agents in every computation step. If this fails at any step,
	the policy falls back to the engine's default scheduling policy.

  \item \asm{onebyone} Executes only one agent in every step. It tries to be {\em fair} by not executing an 
...
\ruleform{\asm{suspend} \asm{value}}{SchedulingPolicies Plugin}\indexrule{suspend}

If \asm{value} is an agent, this rule {\em suspends} the execution of that agent from the next computation step.
The suspended agents will not be chosen by the engine for execution.  

\ruleform{\asm{resume} \asm{value}}{SchedulingPolicies Plugin}\indexrule{resume}
...
\ruleform{\asm{terminate} \asm{value}}{SchedulingPolicies Plugin}\indexrule{terminate}

If \asm{value} is an agent, it will no longer be available for scheduling for the rest of the current run of the machine. 

\ruleform{\asm{shutdown}}{SchedulingPolicies Plugin}\indexrule{shutdown}

Clears the \asm{Agents} universe, such that there will be no agent available to contribute to the 
...
\funcform{$ \mbox{\asm{now}} : \rightarrow \Rdom{Number}$}{Time Plugin}\index{now function@\asm{now} function}

Returns a value representing the current time of the system. Of course, given a step, the value of now is fixed.

\funcform{$ \mbox{\asm{stepcount}} : \rightarrow \Rdom{Number}$}{Time Plugin}\index{stepcount function@\asm{stepcount} function}
...
\ruleform{\asm{debuginfo} \asm{id}\ \asm{value}}{DebugInfo Plugin}\indexrule{debuginfo}
...
\asm{DebugInfo.activeChannels} engine property. This can be done either through the Options plugin or 
...
the evaluation of \asm{debuginfo} results in an empty update set and a print out of the debugging information (if the corresponding 
...
this plugin using the plugin's service interface (see \asm{org.coreasm.engine.plugin.Plugin\#getPluginInterface()}). 
...
%\item {\bf \asm{max(\{v_1, \ldots, v_n\})} }  Returns the maximum value in a collection of numbers. 
%												 If there is one non-number in the collection, it returns \Rundef.
\item {\bf \asm{min(v1, v2)} }   Returns the smaller of two values.
%\item {\bf \asm{min(\{v_1, \ldots, v_n\})} }  Returns the minimum value in a collection of numbers.
%												 If there is one non-number in the collection, it returns \Rundef.
...
%\item {\bf \asm{sum(\{v_1, \ldots, v_n\})} }   This function returns the sum of a collection of numbers. If there is one non-number in the collection, it returns \Rundef.
\end{itemize}
%\end{tabular}
...
%\item {\bf \asm{max(\{v1,\ldots, vn\}, @f)} }  Returns the maximum value in a collection of numbers, 
...
%\item {\bf \asm{min(\{v1,\ldots, vn\}, @f)} }  Returns the minimum value in a collection of numbers, 
%	after applying function {\bf \asm{f}} to the values in the collection. If there is one non-number 
%	in the collection, it returns \Rundef.
...
\begin{coreasmbox}
	...
	\\
	\asm{rule} Main = \\
	\tab \asm{let} e = MathE \asm{in}\\
	\tab \asm{par}\\
	\tab \tab \asm{print} "'e' = " + e\\
	\tab \tab \asm{print}print "log(e) = " + log(e)\\
	\tab \tab \asm{print} "sin(30) = " + round( sin( toRadians(30) ) * 10 ) / 10\\
	\tab \tab \asm{print} "asin(0.5) = " + round( toDegrees( asin(0.5) ) )\\
	\tab \tab \asm{print} "min(51, 43) = " + min(51, 43)\\
	\tab \tab \asm{print} "sum( \{1, 2, 100\} ) = " + sum(\{1, 2, 100\})\\
	\tab \tab \asm{print} "sum( \{1, 2, 100\}, @a ) = " + sum(\{1, 2, 100\}, @a)\\
    %\tab \tab \asm{print} "\{2, 3\} is in P(\{1, 2, 3\}) = " + (\{2, 3\} memberof powerset(\{1,2,3\})) \\
    \tab \tab \asm{choose} x \asm{in} powerset(\{1, 2, 3, 4\}) \asm{do} \\
    \tab \tab \tab \asm{if} x \asm{memberof} powerset(\{1, 2, 3\}) \asm{then} \\
    \tab \tab \tab \tab \asm{print} x + " is a member of powerset(\{1, 2, 3\})"\\
    \tab \tab \tab \asm{else}\\ 
    \tab \tab \tab \tab \asm{print} x + " is not a member of powerset(\{1, 2, 3\})"\\
	\tab \asm{endpar}\\
	\end{spec}
